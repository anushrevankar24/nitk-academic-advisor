...
from ..utils.text_utils import clean_document_text
from ..utils.logger import get_chunk_logger
...
class DocumentProcessor:
    ...
    def __init__(
        ...
        clean_headers: bool = True,  # Enable header cleaning by default
        log_chunks: bool = True  # Enable chunk logging
    ):
        ...
        self.chunk_logger = get_chunk_logger() if log_chunks else None
    ...
    def load_pdfs(self, pdf_dir: Path) -> List[Document]:
        ...
        for pdf_path in tqdm(pdf_files, desc="Loading PDFs"):
            try:
                ...
                # Clean and add metadata to each document
                for doc in docs:
                    original_text = doc.page_content
                    
                    # Clean the document text if header cleaning is enabled
                    if self.clean_headers:
                        cleaned_text = clean_document_text(original_text)
                    else:
                        cleaned_text = original_text
                    
                    # Only keep documents with substantial content after cleaning
                    if len(cleaned_text.strip()) > 50:  # Minimum content threshold
                        ...
                        doc.metadata.update({
                            ...
                            "headers_cleaned": self.clean_headers
                        })
                        documents.append(doc)
                    else:
                        print(f"  Skipped page {doc.metadata.get('page', 'unknown')} from {pdf_path.name} (insufficient content after cleaning)")
                
                print(f"  Loaded {len([d for d in docs if len(d.page_content.strip()) > 50])} pages from {pdf_path.name}")
    ...
    def chunk_documents(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in tqdm(documents, desc="Chunking documents"):
            try:
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if self.log_chunks and self.chunk_logger:
                        source = doc.metadata.get("source", "unknown")
                        page = doc.metadata.get("page", 0)
                        text_preview = chunk.page_content[:200].replace('\n', ' ')
                        if len(chunk.page_content) > 200:
                            text_preview += "..."
                        
                        self.chunk_logger.info(
                            f"CHUNK_LOADED | "
                            f"chunk_id={chunk_id} | "
                            f"source={source} | "
                            f"page={page} | "
                            f"chunk_index={i} | "
                            f"text_length={len(chunk.page_content)} | "
        ...
        if self.log_chunks and self.chunk_logger:
            self.chunk_logger.info(f"TOTAL_CHUNKS_CREATED={len(all_chunks)}")
        return all_chunks
    ...
    def generate_embeddings(self, chunks: List[Document]) -> List[List[float]]:
        ...
        # Add passage prefix for E5 model (same as Embedder class)
        prefixed_texts = [f"passage: {text}" for text in texts]
        
        # Use the same embedding logic as Embedder
        embeddings = self.embeddings.embed_documents(prefixed_texts)
        
        # Validate embedding dimensions
        if embeddings:
            embedding_dim = len(embeddings[0])
            expected_dim = 768  # e5-base-v2 dimension
            if embedding_dim != expected_dim:
                raise ValueError(f"Embedding dimension mismatch: got {embedding_dim}, expected {expected_dim}")
    ...
    def save_to_faiss(
        ...
        # Validate inputs
        if not chunks:
            raise ValueError("No chunks provided for FAISS storage")
        if not embeddings:
            raise ValueError("No embeddings provided for FAISS storage")
        if len(chunks) != len(embeddings):
            raise ValueError(f"Chunks and embeddings count mismatch: {len(chunks)} chunks vs {len(embeddings)} embeddings")
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_metadata = {
                ...
                "page": page_num,
                "page_start": page_num,  # Alias for compatibility
        ...
        if self.log_chunks and self.chunk_logger:
            self.chunk_logger.info(f"FAISS_INDEX_SAVED | total_chunks={len(chunks)} | index_file={index_file}")
    ...
    def save_chunks_to_file(self, chunks: List[Document], output_file: Path) -> None:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            for chunk in tqdm(chunks, desc="Saving chunks"):
                # Create a copy of metadata without chunk_id to avoid duplication
                metadata_copy = chunk.metadata.copy()
                chunk_id = metadata_copy.pop("chunk_id", "")
    ...
    def process_pdfs(self, pdf_dir: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        if self.log_chunks and self.chunk_logger:
            self.chunk_logger.info("=" * 60)
            self.chunk_logger.info("INGESTION_STARTED")
            self.chunk_logger.info(f"PDF_DIR={pdf_dir}")
            self.chunk_logger.info(f"CHUNK_SIZE={self.chunk_size}")
            self.chunk_logger.info(f"CHUNK_OVERLAP={self.chunk_overlap}")
            self.chunk_logger.info(f"CLEAN_HEADERS={self.clean_headers}")
            self.chunk_logger.info("=" * 60)
        ...
        # Flatten metadata to match FAISS structure for consistency
        chunk_dicts = []
        for chunk in chunks:
            chunk_id = chunk.metadata.get("chunk_id", "")
            
            # Flatten metadata - put all fields at top level for consistency with FAISS
            chunk_dict = {
                "chunk_id": chunk_id,
                "text": chunk.page_content,
                "source": chunk.metadata.get("source", "unknown"),
                ...
                "page_start": chunk.metadata.get("page", 0),  # Alias for compatibility
                "page_end": chunk.metadata.get("page", 0),  # Alias for compatibility
            ...
            for key, value in chunk.metadata.items():
                if key not in chunk_dict and key != "chunk_id":
        ...
        if self.log_chunks and self.chunk_logger:
            self.chunk_logger.info(f"BM25_INDEX_SAVED | total_chunks={len(chunk_dicts)} | index_file={BM25_INDEX_FILE}")
